...
const CATEGORY_META = {
  whatsapp:    { label: 'WhatsApp',            subtitle: 'Messaging',        icon: '💬', color: 'basics' },
  payments:    { label: 'Digital Payments',    subtitle: 'Finance',          icon: '💳', color: 'payments' },
  gov:         { label: 'Government Services', subtitle: 'Public Portals',   icon: '🏛️', color: 'gov' },
  social:      { label: 'Social Media',        subtitle: 'Stay Connected',   icon: '📱', color: 'smart' },
  phone:       { label: 'Phone Basics',        subtitle: 'Beginner Journey', icon: '📞', color: 'basics' },
  email:       { label: 'Email & Internet',    subtitle: 'Go Online',        icon: '✉️', color: 'learning' },
  safety:      { label: 'Online Safety',       subtitle: 'Stay Safe',        icon: '🔒', color: 'gov' },
  jobs:        { label: 'Job Applications',    subtitle: 'Career',           icon: '💼', color: 'smart' },
};
...
const CategoryExplore = () => {
  ...
  const meta = CATEGORY_META[categoryKey] || CATEGORY_META.basics;
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        console.log('tutRes full:', JSON.stringify(tutRes));
...
console.log('tutorial category_names:', tutorials.map(t => t.category_name));
  return (
    <>
      {/* ── Header ── */}
      <div className="ce-header">
        ...
        <div className="container-xl">
          ...
          <button className="ce-back" onClick={() => navigate('/seekhna')}>
            <i className="fas fa-arrow-left" /> Back to Seekhna
          </button>
      ...
      {/* ── Grid ── */}
      <div className="container-xl ce-grid-wrap">
        {loading ? (
              ...
              return (
                ...
                >
                  ...
                  <div className="ce-card-footer">
                    <span className="ce-tag" style={{ color: diff.color, borderColor: diff.color }}>
                      <i className="fas fa-signal" /> {diff.label}
                    </span>
                    <span className="ce-tag">
                      <i className="fas fa-clock" /> {t.estimated_time_minutes || 5} min
                    </span>
                  </div>
      ...
      {!loading && filtered.length > 0 && (
        <div className="container-xl ce-tip-wrap">
